...
const COURSE_KEYWORDS: Record<string, string[]> = {
  ...
  // "강제"만 단독 키워드로 있으면 "강제집행"/"강제전학" 같은 무관한 검색어에도
  // 걸리는 오탐이 있었다(2026-09, 버그 감사 중 발견) — 실제로 의도했던 법률
  // 용어인 "강제추행"으로 구체화.
  "성범죄 예방": ["성범죄", "성폭력", "강간", "추행", "강제추행"],
  "성매매 예방": ["성매매", "원조교제", "조건만남"],
  "디지털 성범죄 예방": [
    "디지털", "불법촬영", "몰카", "몰래카메라", "n번방", "딥페이크", "사이버",
  ],
...
function matchesQuery(course: CourseListItem, q: string): boolean {
  ...
  for (const kw of keywords) {
    ...
    // 2) 검색어가 키워드 전체를 포함 (예: q="몰카 범죄" ⊃ kw="몰카").
    // 예전엔 키워드의 앞 2글자만 잘라서 봤는데("강제추행" -> "강제"), 그
    // 2글자가 그 자체로 흔한 단어라서 무관한 검색어("강제집행", "강제전학"
    // 등)에도 걸리는 오탐이 있었다(2026-09, 버그 감사 중 발견) — 키워드
    // 전체가 검색어에 포함되는지로 바꿔 그런 우연한 일치를 없앤다.
