export const metadata = {
  title: "이용약관 | 한국범죄예방교육센터",
};

const ARTICLES: { num: string; title: string; body: string }[] = [
  {
    num: "제1조",
    title: "목적",
    body: `본 이용약관은 "한국범죄예방교육센터"(이하 "사이트")의 서비스의 이용조건과 운영에 관한 제반 사항 규정을 목적으로 합니다.`,
  },
  {
    num: "제2조",
    title: "용어의 정의",
...
⑥ 해지 : 회원이 이용계약을 해약하는 것을 말합니다.`,
  },
  {
    num: "제3조",
    title: "약관 외 준칙",
    body: `운영자는 필요한 경우 별도로 운영정책을 공지 안내할 수 있으며, 본 약관과 운영정책이 중첩될 경우 운영정책이 우선 적용됩니다.`,
  },
  {
    num: "제4조",
    title: "이용계약 체결",
...
운영자는 회원이 미풍양속에 저해되거나 국가안보에 위배되는 게시물 등 위법한 게시물을 등록·배포할 경우 관련 기관의 요청이 있을 시 회원의 자료를 열람 및 해당 자료를 관련 기관에 제출할 수 있습니다.`,
  },
  {
    num: "제7조",
    title: "운영자의 의무",
    body: `① 운영자는 이용회원으로부터 제기되는 의견이나 불만이 정당하다고 인정할 경우에는 가급적 빨리 처리하여야 합니다. 다만, 개인적인 사정으로 신속한 처리가 곤란한 경우에는 사후에 공지 또는 이용회원에게 쪽지, 전자우편 등을 보내는 등 최선을 다합니다.
② 운영자는 계속적이고 안정적인 사이트 제공을 위하여 설비에 장애가 생기거나 유실된 때에는 이를 지체 없이 수리 또는 복구할 수 있도록 사이트에 요구할 수 있습니다. 다만, 천재지변 또는 사이트나 운영자에 부득이한 사유가 있는 경우, 사이트 운영을 일시 정지할 수 있습니다.`,
  },
  {
    num: "제8조",
    title: "회원의 의무",
    body: `① 회원은 본 약관에서 규정하는 사항과 운영자가 정한 제반 규정, 공지사항 및 운영정책 등 사이트가 공지하는 사항 및 관계 법령을 준수하여야 하며, 기타 사이트의 업무에 방해가 되는 행위, 사이트의 명예를 손상하는 행위를 해서는 안 됩니다.
...
④ 회원은 운영자와 사이트 및 제3자의 지적 재산권을 침해해서는 안 됩니다.`,
  },
  {
    num: "제9조",
    title: "서비스 이용 시간",
    body: `① 서비스 이용 시간은 업무상 또는 기술상 특별한 지장이 없는 한 연중무휴 1일 24시간을 원칙으로 합니다. 단, 사이트는 시스템 정기점검, 증설 및 교체를 위해 사이트가 정한 날이나 시간에 서비스를 일시중단 할 수 있으며 예정된 작업으로 인한 서비스 일시 중단은 사이트의 홈페이지에 사전에 공지하오니 수시로 참고하시길 바랍니다.
...
③ 전항에 의한 서비스 중단의 경우 사이트는 사전에 공지사항 등을 통하여 회원에게 통지합니다. 단, 사이트가 통제할 수 없는 사유로 발생한 서비스의 중단에 대하여 사전공지가 불가능한 경우에는 사후공지로 대신합니다.`,
  },
  {
    num: "제10조",
    title: "서비스 이용 해지",
    body: `① 회원이 사이트와의 이용계약을 해지하고자 하는 경우에는 회원 본인이 온라인을 통하여 등록해지 신청을 하여야 합니다. 한편, 사이트 이용 해지와 별개로 사이트에 대한 이용계약 해지는 별도로 하셔야 합니다.
② 해지 신청과 동시에 사이트가 제공하는 사이트 관련 프로그램이 회원 관리 화면에서 자동적으로 삭제됨으로 운영자는 더 이상 해지신청자의 정보를 볼 수 없습니다.`,
  },
  {
    num: "제11조",
    title: "서비스 이용 제한",
    body: `회원은 다음 각호에 해당하는 행위를 하여서는 아니 되며 해당 행위를 한 경우에 사이트는 회원의 서비스 이용 제한 및 적법한 조치를 할 수 있으며 이용계약을 해지하거나 기간을 정하여 서비스를 중지할 수 있습니다.
...
⑧ 기타 관련 법령에 위배되는 행위`,
  },
  {
    num: "제12조",
    title: "게시물의 관리",
    body: `① 사이트의 게시물과 자료의 관리 및 운영의 책임은 운영자에게 있습니다. 운영자는 항상 불량 게시물 및 자료에 대하여 모니터링을 하여야 하며, 불량 게시물 및 자료를 발견하거나 신고를 받으면 해당 게시물 및 자료를 삭제하고 이를 등록한 회원에게 주의를 주어야 합니다.
...
⑨ 운영자는 서버 등 설비의 관리, 점검, 보수, 교체 과정 또는 소프트웨어의 운용 과정에서 고의 또는 고의에 준하는 중대한 과실 없이 발생할 수 있는 시스템의 장애, 제3자의 공격으로 인한 시스템의 장애, 국내외의 저명한 연구기관이나 보안 관련 업체에 의해 대응 방법이 개발되지 아니한 컴퓨터 바이러스 등의 유포나 기타 운영자가 통제할 수 없는 불가항력적 사유로 인한 회원의 손해에 대하여 책임지지 않습니다.`,
  ...
  {
    num: "제17조",
    title: "결제, 서비스 제공기간 및 환불",
    body: `① 본 사이트가 판매하는 온라인 강의는 배송 상품이 아닌 디지털 서비스이며, 수강 가능 기간은 결제일로부터 30일입니다.
② 결제 시스템 오류로 인한 잘못된 결제는 기간에 관계없이 환불 또는 취소가 가능합니다.
③ 온라인 강의는 결제 후 7일 이내이고 강의 수강을 시작하지 않은 경우 전액 환불이 가능하며, 강의 수강을 시작한 이후에는 환불이 불가합니다.
④ 심리상담(서면 심리상담 의견서, 전화 심화상담 의견서)은 결제 후 7일 이내이고 상담 의뢰(설문 제출)를 하지 않은 경우 전액 환불이 가능하며, 상담 의뢰 이후에는 환불이 불가합니다.
⑤ 반성문·탄원서 작성 서비스는 결제 후 7일 이내이고 작성 신청(내용 제출)을 하지 않은 경우 전액 환불이 가능하며, 작성 신청 이후에는 환불이 불가합니다.
⑥ 구속수용자 교육은 신청 즉시 교육자료 발송 준비에 들어가므로 신청·결제 후에는 환불이 어렵습니다.
⑦ 환불 문의는 admin@kcpec.co.kr 로 접수하며, 환불은 결제하신 수단으로 취소 처리됩니다.`,
  },
];
  ...
  return (
    <div className="bg-white">
      <article className="mx-auto max-w-[800px] px-6 py-16">
          ...
          {ARTICLES.map((a) => (
            <section key={a.num}>
              <h2 className="font-sans text-lg font-bold text-slate-900 sm:text-xl">
                <span className="mr-2 text-[var(--color-primary)]">{a.num}</span>
                {a.title}
              </h2>
              <div className="mt-3 whitespace-pre-line text-[15px] leading-[1.85] text-slate-700">
                {a.body}
              </div>
            </section>
          ))}
        ...
        <footer className="mt-16 border-t border-zinc-200 pt-6 text-sm text-slate-500">
          부칙: 이 약관은 2024년 5월 1일부터 시행합니다. 제17조(결제, 서비스 제공기간 및 환불)는 2026년 9월 25일부터 시행합니다.
        </footer>
      </article>
    </div>
  );
